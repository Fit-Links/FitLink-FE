...
const myInformationQueries = {
  all: () => ["myInformation"],
  summary: () =>
    queryOptions({
      queryKey: [...myInformationQueries.all(), "summary"],
      queryFn: getMyInformation,
    }),
  detail: () =>
    queryOptions({
      queryKey: [...myInformationQueries.all(), "detail"],
      queryFn: getMyInformationDetail,
    }),
  ptHistories: () => [...myInformationQueries.all(), "ptHistory"],
  ptHistory: (memberId: number, status?: MyPtHistoryStatus) =>
    infiniteQueryOptions({
      queryKey: [...myInformationQueries.ptHistories(), memberId, status],
      queryFn: ({ pageParam }) =>
...
export default myInformationQueries;
